...
class Bool: 
    ...
    async def complete_daily_task(self, task_id: int):
        json_data = self.payload.copy()
        json_data['assignmentId'] = task_id

        r = await (await self.session.post('https://miniapp.bool.network/backend/bool-tg-interface/assignment/daily/do', json=json_data)).json()
        return r.get('data') is True

    async def get_daily_tasks(self):
        r = await (await self.session.post('https://miniapp.bool.network/backend/bool-tg-interface/assignment/daily/list', json=self.payload)).json()
        return r.get('data')
